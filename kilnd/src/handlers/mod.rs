...
pub fn route(store: &Store, req: &Request, stream: &mut Conn, reader: &mut BufReader<Conn>) -> io::Result<()> {
    ...
    match (req.method.as_str(), segments.as_slice()) {
        ...
        ("GET", ["containers", id, "resources"]) => containers::resources(store, id).write_to(stream),
        ...
        ("GET", ["volumes", name, "snapshots"]) => volumes::list_snapshots(store, name).write_to(stream),
        ("POST", ["volumes", name, "snapshots"]) => volumes::create_snapshot(store, name, req).write_to(stream),
        ("POST", ["volumes", name, "snapshots", snapshot_id, "restore"]) => volumes::restore_snapshot(store, name, snapshot_id).write_to(stream),
        ...
        ("POST", ["secrets", name, "rotate"]) => secrets::rotate(store, name, req).write_to(stream),
